...
def rebuild_tree(trj_cnt,P_c,tree,N):
	...
	for l in range(len(tree)):
		...
		if tree[l].left>=0: 
			len_l=len(Ixs_c[tree[l].left])
		else: 
			len_l=len(Ixs_c[-1*tree[l].left+trj_cnt-1])

		if tree[l].right>=0: 
			len_r=len(Ixs_c[tree[l].right])
		else: 
			len_r=len(Ixs_c[-1*tree[l].right+trj_cnt-1])

		n=len_l+len_r	#number of particles for class c
...
def mpf(lbl,coarse_ratio,coarse_and_fine,trj_cnt,observation_noise,abrupt,once,once_gamma,abrupt_ratio,write_files,print_out, dynamics_noise=0.01, convergence_time_mode=0, dist_threshold=0.1,abrupt_once=0):
	...
	if frechet_clust:	
		...
		[tree_full, C_full, parent_full]=cluster_frechet_hierarchical.cluster_frechet(dist_file,-1)	# the full tree
		#[tree, C, parent]=cluster_frechet_hierarchical.cluster_frechet(dist_file, lbl)	# the tree without the test trajectory
	...
	for id, n in enumerate(tree): #id  will be 0 for -1, 1 for -2,... -1*id-1 to get the cluster number
	    if n.left>=0:	#normal trajectory
		if I[n.left]==n.left:	#not changed yet
			I[n.left]=id+1
	    else:		#a cluster already
		if I[-1*n.left+trj_cnt-1]==-1*n.left+trj_cnt-1:	#not changed yet
			I[-1*n.left+trj_cnt-1]=id+1
	    if n.right>=0:	#normal trajectory
		if I[n.right]==n.right:	#not changed yet
			I[n.right]=id+1
	    else:		#a cluster already
		if I[-1*n.right+trj_cnt-1]==-1*n.right+trj_cnt-1:	#not changed yet
			I[-1*n.right+trj_cnt-1]=id+1
	...
	for n in tree: #id  will be 0 for -1, 1 for -2,... -1*id-1 to get the cluster number
		...
		if I[n.left]==n.left:	#not changed yet
			I[n.left]=id+1
			death_d[n.left]=n.distance
	    else:		#a cluster already
		if I[-1*n.left+trj_cnt-1]==-1*n.left+trj_cnt-1:	#not changed yet
			I[-1*n.left+trj_cnt-1]=id+1
			death_d[-1*n.left+trj_cnt-1]=n.distance
	    if n.right>=0:	#normal trajectory
		if I[n.right]==n.right:	#not changed yet
			I[n.right]=id+1
			death_d[n.right]=n.distance
	    else:		#a cluster already
		if I[-1*n.right+trj_cnt-1]==-1*n.right+trj_cnt-1:	#not changed yet
			I[-1*n.right+trj_cnt-1]=id+1
			death_d[-1*n.right+trj_cnt-1]=n.distance
	...
	for t in range(T):
		...
		for j in sorted([j for j in range(len(tree)) if birth_d[trj_cnt+j]>gamma]):
			...
			P[trj_cnt+j]=(P[tree[j].left] if tree[j].left>=0 else P[-1*tree[j].left+trj_cnt-1])+(P[tree[j].right]  if tree[j].right>=0 else P[-1*tree[j].right+trj_cnt-1])
		...
		for j in [j for j in range(trj_cnt+len(tree)-2,-1,-1) if death_d[j]<=gamma]: 	#go through the tree, j=len(tree) --> j=1
			...
			children=[tree[prnt-trj_cnt].right if tree[prnt-trj_cnt].right>=0 else -1*tree[prnt-trj_cnt].right+trj_cnt-1, tree[prnt-trj_cnt].left if tree[prnt-trj_cnt].left>=0 else -1*tree[prnt-trj_cnt].left+trj_cnt-1]
		...
		X0+=x0
		map_dist_tot+=max([  round(class_dist1(x0_,lbl,parent_full, trj_cnt,tree_full),2) for x0_ in x0  ])
		
		if len(map_dist_win5)>=5: 
			del map_dist_win5[0] 	#remove the oldest element
		map_dist_win5+=[max([  round(class_dist1(x0_,lbl,parent_full, trj_cnt,tree_full),2) for x0_ in x0  ])]
		...
		if t%5==0:   #every 5 timesteps
			...
			if l==0 : 
				...
				if lbl is not None:
					map_dists+=[round(class_dist1(P_g_i[0],lbl,parent_full, trj_cnt,tree_full),2)]#distance to MAP class at g
			
				sum_dists=0.0			
				for h in range(len(P_g_i) ):
					if lbl is not None:
						sum_dists+=P_g_s[h]*class_dist2(P_g_i[h],lbl,parent_full, trj_cnt+1,tree_full)
